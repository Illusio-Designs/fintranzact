/**
 * Resolve an API path to a full URL based on the build-time API_URL.
 *
 * - When API_URL is unset (dev + single-origin deploys): returns the
 *   path unchanged, relying on Vite's dev-server proxy or same-origin nginx.
 * - When API_URL is set (split-host deploys like app.hisaabo.in +
...
export function apiUrl(path: string): string {
  const base = import.meta.env.API_URL as string | undefined;
